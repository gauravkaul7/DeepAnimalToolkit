...
class KeypointTrainer(DefaultTrainer):
    ...
    """This is a coustom trainer for a keypoint dataset,
    the standard trainer flips certain keypoints as an augmentation,
    but this assumes that fliping the image + keypoints annotations
    preserve the keypoint mapping we are trying to learn.
    This is usually not the case for animals, especially when
    the camera view is a birds eye view"""
...
class DetectorTrainer:
    ...
    def load_dataset(path):
        ...
        def get_mouse_dicts(annotations_path):
            data_path = "/content/drive/MyDrive/keypoint_dataset/datasets/"
            dataset_dicts = [
                json.load(open(data_path + annotations_path + x))
                for x in os.listdir(data_path + annotations_path)
                if x[-5:] == ".json"
            ]
        ...
        DatasetCatalog.register(
            "mouse_train", lambda p=annotations_folder: get_mouse_dicts(p)
        )
        ...
        MetadataCatalog.get("mouse_train").set(
